...
void TurretSubsystem::runTurret() {
	...
	monitorTimeStart = Timer::GetFPGATimestamp();
	while(runThread) {
		...
		if (Timer::GetFPGATimestamp() - monitorTimeStart > 0.25) {
			currEncoder = turretMotor->GetPosition();
			if (abs(turretMotor->GetOutputCurrent()) > 0.5) {
				if (prevEncoder == currEncoder) {
					turretErrorCounter++;
				} else {
					turretErrorCounter = 0;
				}
...
bool TurretSubsystem::hasError() {
	return turretError;
}

void TurretSubsystem::resetTurret() {
	turretErrorCounter = 0;
	turretError = false;
	init();
}
