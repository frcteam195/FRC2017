...
HIDControllerSubsystem::HIDControllerSubsystem(Controllers *robotControllers, vector<CustomSubsystem*> *subsystemVector) {
	...
	gearOscillatorAction = new GearOscillatorAction(robotControllers, subsystemVector);
	...
	targetSearchAction = new TargetSearchAction(robotControllers, subsystemVector);
	triggerClimbAction = new TriggerClimbAction(robotControllers, subsystemVector);
...
void HIDControllerSubsystem::runDriveJoystickThread() {
	...
	while(runThread) {
		...
		if(driveJoystick->GetRawButton(DRIVE_AGITATE)) {
			agitateAction->start();
		}
...
void HIDControllerSubsystem::runArmJoystickThread() {
	...
	while(runThread) {
		...
		//	Parade Code
		/*if(armJoystick->GetRawAxis(2) < 0.5) {
		...
		if (armJoystick->GetRisingEdgeButton(SPEED_UP)) {
			visionTurretControlAction->setManualAdjust(true);
		...
		if (armJoystick->GetRawButton(MANUAL_OVERRIDE_BUTTON)) {
			...
			manualTurretControlAction->stop();
...
void HIDControllerSubsystem::runButtonBoxThread() {
	...
	while(runThread) {
		...
		if (buttonBox2->GetRawButton(CLIMB_BUTTON)) {
			if (ds->GetMatchTime() <= 30) {
				climbAction->start();
			}
		} else if (driveJoystick->GetRawButton(TRIGGER_CLIMB_BUTTON)) {
			triggerClimbAction->start();
		} else if (buttonBox1->GetRawButton(INTAKE_OUT_BUTTON)) {
			intakeOutAction->start();
		} else if (buttonBox1->GetRawButton(INTAKE_IN_BUTTON) || armJoystick->GetRawButton(ARM_INTAKE_IN_BUTTON) || driveJoystick->GetRawAxis(2) < -0.5 || armJoystick->GetRawAxis(3) < -0.5) {
		...
		if (buttonBox1->GetRisingEdgeButton(SHOOT_FROM_HOPPER)) {
			...
			visionTurretControlAction->start();
		} else if(buttonBox2->GetRisingEdgeButton(SHOOT_FROM_LAUNCHPAD_LINE)) {
			visionTurretControlAction->stop();
			shooterAction->start(ShotSelection::launchPadLine);
			visionTurretControlAction->start();
		} else if (buttonBox2->GetRisingEdgeButton(SHOOT_SEARCH_AND_DESTROY)) {
			visionTurretControlAction->stop();
			targetSearchAction->start();
			visionTurretControlAction->setManualAdjust(false);
			visionTurretControlAction->clearBuffer();
			visionTurretControlAction->start(true);
		} /*else if(armJoystick->GetRisingEdgeButton(VISION_SET_ON)) {
			visionTurretControlAction->stop();
			shooterAction->start(ShotSelection::paradeShot);
		}*/
