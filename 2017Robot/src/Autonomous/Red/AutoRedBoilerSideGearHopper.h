/*
 * AutoRedBoilerSideGearHopper.h
 *
 *  Created on: Mar 14, 2017
 *      Author: roberthilton
 */

#ifndef SRC_AUTONOMOUS_AUTOREDBOILERSIDEGEARHOPPER_H_
#define SRC_AUTONOMOUS_AUTOREDBOILERSIDEGEARHOPPER_H_
...
#define AUTO_HOPPER_GEAR_RED_TARGETING_TIMEOUT_MS 750
#define AUTO_SIDE_GEAR_PLACING_TIMEOUT_MS 1500

class AutoRedBoilerSideGearHopper: public CustomAction {
public:
	AutoRedBoilerSideGearHopper(Controllers *robotControllers, vector<CustomSubsystem*> *subsystemVector)
	...
	void start() override {
		...
		do {
			...
			this_thread::sleep_for(chrono::milliseconds(10));
		} while (avgPosTmp < 48 && ds->IsAutonomous());
		...
		while (ontargetFilter < 4 && timeoutElapsedTimeMS < AUTO_HOPPER_GEAR_RED_TARGETING_TIMEOUT_MS) {
			this_thread::sleep_for(chrono::milliseconds(25));
			timeoutEnd = Timer::GetFPGATimestamp();
			timeoutElapsedTimeMS = (int)((timeoutEnd - timeoutStart) * 1000);
...
#endif /* SRC_AUTONOMOUS_AUTOREDBOILERSIDEGEARHOPPER_H_ */
