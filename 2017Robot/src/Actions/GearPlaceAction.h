...
protected:
	void run() override {
		...
		do {
			...
			if (avgPosTmp > 0.18) {
				gearSubsystem->setGearFingers(true);
				gearSubsystem->setGearClamp(true);
				gearSubsystem->setGearPusher(true);
				//gearSubsystem->setGearActuatorPos(POSITION_ZERO-0.07);

				gearSubsystem->setGearActuatorPos(POSITION_ZERO-0.14);
			}
		} while (avgPosTmp < 0.25 && timeoutElapsedTimeMS < GEAR_DRIVE_TIMEOUT_MS);
		...
		gearSubsystem->setGearFingers(false);
		//driveBaseSubsystem->setDriveSpeed(1, -1);
