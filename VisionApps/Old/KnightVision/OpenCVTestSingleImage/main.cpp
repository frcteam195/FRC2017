...
#else
	Mat frame = imread("/Users/roberthilton/Desktop/imageStore/ImgOri64.jpg");
#endif
 ...
 */
	...
	cv::Point2f pts[4];
	if (filteredBoxes.size() == 2) {
		if (filteredBoxes[0].size.area() > filteredBoxes[1].size.area()) {
			filteredBoxes[0].points(pts);
		} else {
			filteredBoxes[1].points(pts);
		}
	}
	...
	int maxY = -10000;
	for (int i = 0; i < 4; i++) {
		minX = pts[i].x > minX ? minX : pts[i].x;
		minY = pts[i].y > minY ? minY : pts[i].y;
		maxX = pts[i].x < maxX ? maxX : pts[i].x;
		maxY = pts[i].y < maxY ? maxY : pts[i].y;
	}
	
	int interestRegionMinX = (maxX + minX)/2 - 5;
	int interestRegionMaxX = (maxX + minX)/2 + 5;
	int interestRegionMinY = (maxY + minY)/2 - 2;
	int interestRegionMaxY = (maxY + minY)/2 + 2;
	
	double t = 0;
	int counter = 0;
	const float scale = 0.1;
	
	for (int i = interestRegionMinX; i < interestRegionMaxX; i++) {
		for (int j = interestRegionMinY; j < interestRegionMaxY; j++) {
			t += scale * frame.at<uint16_t>(cv::Point(i, j));
			counter++;
		}
	}
	t /= counter;
	cout << "Average Point Depth: " << t << endl;
	
	
	cout << "min x: " << interestRegionMinX << endl;
	cout << "max x: " << interestRegionMaxX << endl;
	cout << "min y: " << interestRegionMinY << endl;
	cout << "max y: " << interestRegionMaxY << endl;
	...
	/*
	 cout << "Pt 0 x: " << pts[0].x << endl;
	 cout << "Pt 0 y: " << pts[0].y << endl;
	 cout << "Pt 1 x: " << pts[1].x << endl;
	 cout << "Pt 1 y: " << pts[1].y << endl;
	 cout << "Pt 2 x: " << pts[2].x << endl;
	 cout << "Pt 2 y: " << pts[2].y << endl;
	 cout << "Pt 3 x: " << pts[3].x << endl;
	 cout << "Pt 3 y: " << pts[3].y << endl;
	 */
